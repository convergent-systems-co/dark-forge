...
**Review prompts** are consolidated, self-contained review definitions in `governance/prompts/reviews/`. There are 21 review prompts implementing Anthropic's Parallelization (Voting) pattern. Each prompt inlines its participant perspectives with full evaluation criteria, scoring, and output schema. Additionally, 7 agentic personas (Project Manager, DevOps Engineer, Code Manager, Coder, IaC Engineer, Tester, Document Writer) in `governance/personas/agentic/` drive the autonomous pipeline.
